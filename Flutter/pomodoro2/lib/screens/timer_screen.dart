...
class _TimerScreenState extends State<TimerScreen> {
      ...
      switch(_timerStatus){
        ...
        case TimerStatus.resting:
          if(_timer<=0){
            ...
            showToast("오늘 $_pomodoroCount개의 뽀모도로를 달성함.");
            t.cancel();
            stop();
          }else{
            setState(() {
  ...
  Widget build(BuildContext context) {
    final List<Widget> _runningButtons = [
      ElevatedButton(
        child: Text(_timerStatus==TimerStatus.paused?'계속하기':'일시정지', style: TextStyle(color: Colors.white, fontSize: 16),),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Center(child: Text('뽀모도로 타이머')),
